...
export default function Participants({ id }: { id: string }) {
  ...
  const renderCustomTable = () => {
    ...
    return (
      <div>
        <h1 className="text-3xl mb-4 mt-8 font-bold">Danh sách thí sinh tham gia</h1>
              ...
              <tr key={rowIndex}>
                {row.map((participant, colIndex) => (
                  <td key={colIndex} className="border border-gray-300 p-4">
                    <div className="flex justify-between items-center">
                      ...
                      <div>
                        {participant.challenge?.status === 'I' ? (
                          <Button
                            className="border border-gray-300 bg-[#eeeeee] text-black hover:text-white"
                            onClick={() => viewRound(participant.challenge?.round)}
                          >
                            Xem trận đấu
                          </Button>
                        ) : participant.challenge?.first_user === user?.id ? (
                          <Button onClick={() => cancelChallenge(participant.challenge.id)} className="bg-[#FF8B3E]">
                            Hủy thách đấu
                          </Button>
                          ...
                          <>
                            ...
                            <Button
                              onClick={() => rejectChallenge(participant.challenge.id)}
                              className="ml-2 bg-[#e03444]"
                            >
                              Từ chối
                            </Button>
                          </>
                          ...
                          !user?.is_admin &&
                          tournament?.tournament.problem != undefined ? (
                          <Button
                            onClick={() => createChallenge(participant.id, tournament?.tournament.problem)}
                            className="bg-[#1C81BA]"
                          >
                            Thách đấu
                          </Button>
                ...
                {row.length < 3 &&
                  Array.from({ length: 3 - row.length }).map((_, colIndex) => (
                    <td key={`empty-${colIndex}`} className="border border-gray-300 p-2 text-center" />
                  ))}
              </tr>
            ))}
          </tbody>
